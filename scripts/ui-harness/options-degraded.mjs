...
const temporaryRoot = await mkdtemp(
  path.join(os.tmpdir(), "trackerblocker-ui-degraded-"),
);
...
try {
  ...
  process.exitCode = await runOptionsTest();
} finally {
...
function runOptionsTest() {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(
      process.execPath,
      [path.join(ROOT, "scripts/ui-harness/options.test.mjs")],
      {
        cwd: ROOT,
        env: {
          ...process.env,
          EXPECT_ENGINE_HEALTHY: "false",
          UI_HARNESS_EXTENSION_SOURCE: temporarySource,
        },
        stdio: "inherit",
      },
    );
    ...
    child.once("exit", (code, signal) => {
      signal
        ? reject(new Error(`Degraded options UI test stopped by ${signal}.`))
